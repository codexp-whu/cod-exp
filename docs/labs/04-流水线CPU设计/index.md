...
!!! warning "Chisel入门"
    从本栏开始，CPU 改用 **Chisel** 开发，请配合 [Chisel入门](../../chisel/简介.md) 讲义配置好Chisel开发环境并学习基本语法。

实验六中我们已经使用Verilog完成了一个支持20条指令的单周期CPU，接下来我们将实现一个五级流水线CPU。

五级流水线把单周期数据通路切分为 IF / ID / EX / MEM / WB 五级，用流水级寄存器衔接。指令在各级并行执行，吞吐率提升数倍，但**相关与冲突**也随之而来。

本栏实验，就是先搭建一个五级流水线框架（实验七），再用阻塞和前递解决冲突（实验八、九），最后扩展指令系统（实验十、十一）。

## 实验列表

| 实验 | 主题 | 原书参考 |
| ---- | ---- | ---- |
| [实验7：不考虑相关冲突处理的简单流水线CPU](./实验7-简单流水线CPU.md) | 搭建五级流水线框架 | [5.1.1 实践任务7](https://bookdown.org/loongson/_book3/chapter-simple-pipeline-cpu.html) |
